...
(function withAngular(angular, window) {
  ...
  , getAttributesToAdd = function getAttributesToAdd(element) {
    ...
    if (element.attr('tooltip-template') !== undefined) {
      ...
      if (element.attr('tooltip-template-array') !== undefined) {

          attributesToAdd['tooltip-template-array'] = element.attr('tooltip-template-array');
          element.removeAttr('tooltip-template-array');
      }
  ...
  , tooltipDirective = /*@ngInject*/ ["$log", "$http", "$compile", "$timeout", "$controller", "$injector", "tooltipsConf", function tooltipDirective($log, $http, $compile, $timeout, $controller, $injector, tooltipsConf) {
    ...
    var linkingFunction = function linkingFunction($scope, $element, $attrs, $controllerDirective, $transcludeFunc) {
      ...
      if ($attrs.tooltipTemplate &&
        ...
        if ($attrs.tooltipTemplate &&
            $attrs.tooltipTemplateArray) {

            throw new Error('You can not define tooltip-template and tooltip-array together');
        }
      ...
      $transcludeFunc($scope, function onTransclusionDone(element, scope) {
        var attributes = getAttributesToAdd(element)
          ...
          , onTooltipTemplateChange = function onTooltipTemplateChange(newValue) {
            ...
            , onTooltipTemplateArrayChange = function onTooltipTemplateArrayChange(newValue) {

                if (newValue) {
                    console.log("onTooltipTemplateArrayChange");
                    console.log(JSON.parse(newValue));
                    tipTipElement.empty();
                    tipTipElement.append(closeButtonElement);
                    angular.forEach(JSON.parse(newValue), function(row, key) {
                        tipTipElement.append(row.message);
                        tipTipElement.append("</br>");
                        tipTipElement.append("</br>");
                    })
                    $timeout(function doLater() {
          ...
          , unregisterOnTooltipTemplateArrayChange = $attrs.$observe('tooltipTemplateArray', onTooltipTemplateArrayChange)
